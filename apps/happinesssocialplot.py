...
layout = html.Div([
    html.Div([
        ...
        dcc.Graph(id="happiness-social-graph"),
        ]),
        dbc.Container([
            dbc.Row([
                dbc.Col(html.Li(children="Happier countries are more likely going to have access to social support such as family, friends, and social services."), className="mb-2")
            ])
        ])
    ]),
])
...
@app.callback(
    Output("happiness-social-graph", "figure"), 
    [Input("dropdown", "value")])

def update_scatter_chart(year):
    if year == 2015:
        fig = px.scatter(df_2015, x='Family', y='Happiness Score', hover_name ="Country", trendline="lowess")
    elif year == 2016:
        fig = px.scatter(df_2016, x='Family', y='Happiness Score', hover_name ="Country", trendline="lowess")
    elif year == 2017:
        fig = px.scatter(df_2017, x='Family', y='Happiness Score', hover_name ="Country", trendline="lowess") 
    elif year == 2018:
        fig = px.scatter(df_2018, x='Family', y='Happiness Score', hover_name ="Country", trendline="lowess") 
    elif year == 2019:
        fig = px.scatter(df_2019, x='Family', y='Happiness Score', hover_name ="Country", trendline="lowess")
    elif year == 2020:
        fig = px.scatter(df_2020, x='Family', y='Happiness Score', hover_name ="Country", trendline="lowess")
