...
layout = html.Div([
    html.Div([
        ...
        dcc.Graph(id="happiness-healthy-graph"),
        ]),
        dbc.Container([
            dbc.Row([
                dbc.Col(html.Li(children="Countries with a higher life expectancy to tend to be happier."), className="mb-2")
            ])
        ])
    ]),
])
...
@app.callback(
    Output("happiness-healthy-graph", "figure"), 
    [Input("dropdown", "value")])

def update_scatter_chart(year):
    if year == 2015:
        fig = px.scatter(df_2015, x='Health (Life Expectancy)', y='Happiness Score', hover_name ="Country", trendline="lowess")
    elif year == 2016:
        fig = px.scatter(df_2016, x='Health (Life Expectancy)', y='Happiness Score', hover_name ="Country", trendline="lowess")
    elif year == 2017:
        fig = px.scatter(df_2017, x='Health (Life Expectancy)', y='Happiness Score', hover_name ="Country", trendline="lowess") 
    elif year == 2018:
        fig = px.scatter(df_2018, x='Health (Life Expectancy)', y='Happiness Score', hover_name ="Country", trendline="lowess") 
    elif year == 2019:
        fig = px.scatter(df_2019, x='Health (Life Expectancy)', y='Happiness Score', hover_name ="Country", trendline="lowess")
    elif year == 2020:
        fig = px.scatter(df_2020, x='Health (Life Expectancy)', y='Happiness Score', hover_name ="Country", trendline="lowess")
